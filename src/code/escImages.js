        ...
        for (const src of this.#uniqueImages) {
            ...
            const promise = new Promise((resolve) => {
                ...
                img.onerror = () => {
                    ...
                    fallbackImg.onerror = () => {
                        // The default image isn't there under the current install folder either —
                        // retry once more under the legacy HACS folder name, for users who haven't
                        // reinstalled under the renamed repo yet. Remove alongside the other legacy
                        // migration shims once installs have caught up.
                        const legacySrc = `${C.ESC_IMAGE_MAP_LEGACY}/${defaultFile}`;
                        const legacyImg = new Image();
                        legacyImg.onload = () => {
                            this.#dimensions.set(src, new xyPair(legacyImg.width, legacyImg.height));
                            this.#resolvedSrc.set(src, legacySrc); // ← remap src
                            resolve();
                        };
                        legacyImg.onerror = () => {
                            // Every fallback failed — store zero size and move on.
                            // Never reject: we want Promise.all to load as much as possible.
                            this.#dimensions.set(src, new xyPair(0, 0));
                            this.#resolvedSrc.set(src, legacySrc); // ← remap src
                            resolve();
                        };
                        legacyImg.src = legacySrc;
                    };
